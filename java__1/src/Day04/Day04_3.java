...
public class Day04_3 {
		...
		// console 키오스크 [ 자판기 ]
			// 1. 메뉴판 [ 1.콜라[300] 2.환타[200] 3.사이다[100] 4.결제 ]
				// 메뉴판 아래에 선택한 제품들 목록 [ 제품수량 / 총 결제 예정 금액 ]
			// 2. 초기 재고 [ 10개씩 ] 팔렸을때 재고 감소 // 재고가 없으면 구매불가
			// 3. 결제를 금액을 입력받아서 결재액만큼 차감후 잔돈 출력	
	public static void main(String[] args) {
		
		Scanner scanner = new Scanner(System.in);
		// 재고변수[지역변수: 선언된 괄호 안에서 사용 가능하다]
		...
		while(true) {
			...
			if( ch == 1) { // 1번 콜라를 선택 했을때
				// 재고가 없을경우 구매 불가
				if( 콜라재고 == 0 ) { System.out.println(" [[콜라 재고가 없습니다 : 제품준비중 ]] ");	}
				else {
			...
			if( ch == 2) { // 2번 환타를 선택 했을때
				// 재고가 없을경우 구매 불가
				if( 환타재고 == 0 ) { System.out.println(" [[환타 재고가 없습니다 : 제품준비중 ]] ");	}
				else {
			...
			if( ch == 3) { // 3번 사이다를 선택 했을때
				// 재고가 없을경우 구매 불가
				if( 사이다재고 == 0 ) { System.out.println(" [[사이다 재고가 없습니다 : 제품준비중 ]] ");	}
				else {
			...
			System.out.println(" 제품명\t제품수량\t결재액");
			...
			System.out.println(" >>>>>> 총 결재액 " + ( (콜라구매수*300) + (환타구매수*200) + (사이다구매수*100) ) );
			System.out.println("---------------------------------\n");
