...
public class BankApplication {
		...
		// 접근제한자
			// public : 프로젝트 내 모든곳에서 호출 가능 
			// private : 현재 클래스 에서만 호출가능
			// default[생략] : 동일한 패키지내애서만 호출 가능
			// protected : 동일한 패키지내에서만 호출 가능
		
		//100개 계좌객체를 저장할수있는 배열 선언
	...
	//메소드선언
	// 접근제한자 키워드 반환타입 메소드명( 매개변수, 매개변수2 ~)
	//static 사용이유 : main에서 호출하기 위해 [ 미사용시 : main 해당 메소드사용불가 ]
	...
	private static Account findAccount(String ano) {
		...
		for(int i =0 ; i<accountArray.length ; i++ ) {
			if( accountArray[i] !=null ) { // i번쨰 인덱스가 공백이 아니면서
	...
	private static void withdraw() {
		...
		if(account == null ) { System.out.println("결과 : 계좌가 존재하지않습니다"); return; }
		if(account.getBalance() < balance ) {
			System.out.println("예금액보다 많은 금액을 출금할 수 없습니다"); return;
		
			// 출금액이 기존예금액보다 크면 출금불가
		}
		// 4. 기존예금액 - 출금액  
		account.setBalance(account.getBalance() - balance) ;
