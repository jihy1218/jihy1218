...
public class Member { // 회원 클래스 설계
	...
	public static boolean signup() {
		// public : main 속한 클래스 ( 외부클래스 )로 호출하기 위해
		...
		System.out.println(" 아이디 : "); 	String m_id = BookApplication.scanner.next();
			...
			if(check) {
				System.out.println(" [[알림]] : 사용 할 수 없는 아이디 입니다 ");
		...
		for( int i = 0 ; i<BookApplication.members.length ; i++ ) {
			if( BookApplication.members[i] == null ) {
				...
				return true; // 회원가입 성공 	// 반환값 : 회원가입 성공/실패 => True / False
				
			}
		}
		
		return false; // 회원가입 실패 [ 빈공간이 없어서 ]
	}
	// 6. 아이디 체크 메소드
	...
	public static String login() {
		...
		return null; // 로그인실패  // 반환값 : 로그인 성공시 => 성공한 아이디 / 실패시 null 
	}
	...
	public static String findid() {
		...
		System.out.println(" 연락처 : "); String m_phone = BookApplication.scanner.next();
		// 2. 배열내 존재 하면 아이디 찾기 성공 
		for( int i = 0; 1<BookApplication.members.length; i++) {
			if(BookApplication.members[i] != null &&
	...
	public static String findpw() {
		...
		for( int i = 0; 1<BookApplication.members.length; i++) {
			if(BookApplication.members[i] != null &&
					BookApplication.members[i].getM_name().equals(m_id) &&
					BookApplication.members[i].getM_phone().equals(m_phone)) {
				return BookApplication.members[i].getM_pw(); //비밀번호 찾기 성공
