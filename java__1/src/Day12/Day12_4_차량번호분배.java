...
public class Day12_4_차량번호분배 {
	...
	// 문제: 차량번호 랜덤으로 0000~9999 사이의 난수10개 생성
	// 1. 모든차량을 배열에 저장
	// *끝자리 번호 홀수/짝수
	// 2. 차량끝자리 번호가 홀수 배열 출력
	// 3. 차량끝자리 번호가 짝수 배열 출력
	// 3개의 배열 모두 출력
	...
	public static void main(String[] args) {
		...
		String[] carnumbers = new String[10];	// String 10개 저장할수있는 배열
			...
			if(ch==1) {
				...
				for(int i = 0 ; i <=carnumbers.length; i++) {
					if(carnumbers[i] == null) {
						carnumbers[i] = strnum; 
						// 홀 짝 구분 [ 값%2 == 0 나머지가 0이면 짝수 나머지가 1이면 홀수
							// 1. 문자 -> 정수 변환  		2. 홀 짝 구분
				...
				for(String num : carnumbers ) {   //인덱스 번호가 필요없는 반복 
					// for(자료형 임시변수 : 배열명) : 
					// 배열내 0번 인덱스 부터 마지막인덱스까지 임시변수에 대입
