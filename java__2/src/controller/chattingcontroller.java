...
public class chattingcontroller implements Initializable{
		...
		Thread thread = new Thread() {
			...
			public void run() {
				try {
					...
					outputStream.write(msg.getBytes());						// 2. 인수로 받은 메시지를 바이트형 변환해서 보내기 
					outputStream.flush();	
				} 
				catch (Exception e) {}
			}
	...
	public void receive() {
		while(true) {
			try {
				...
				inputStream.read(bytes);								// 2. 입력스트림내 바이트를 가져오기
	...
	void connect(ActionEvent event) {
		if(btnconnect.getText().equals("접속하기")) {
			...
			clientstart();
			// 2. 접속 메시지 전달
			Platform.runLater( ()-> txtclient.appendText("---{채팅방 입장}---\n") );
			// 3. 컨트롤 내욜 변경
